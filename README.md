...
<p>
В текущей редакции реализован простейший мост Wi-Fi-UART: открыв в браузере страницу, например, http://192.168.4.1/CTGET=STAT|FREERAM - можно посмотреть, как ответит контроллер на этот запрос. В частности, в ответ на указанный вернётся количество свободной оперативной памяти контроллера. В следующих редакциях функционал будет расширяться.
